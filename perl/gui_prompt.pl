...
sub register_wndclass {
    ...
    my $wndproc_ref = sub {
        ...
        print STDERR "wndproc_ref($hwnd, $uMsg, $wParam, $lParam)\n";
    ...
    my $cbWndExtra = 0;
    my $hIcon = 0;
    my $hCursor = 0; # $LoadCursorW->Call(0, to_wide_bytes(0x7F00)); # IDC_ARROW = MAKEINTRESOURCE(32512) but easier to pass 0x7F00? Might be inconsistent.
...
#    $hbrBackground = 0;
    ...
    my $packed = pack('L Q l l Q Q Q Q Q Q',
        0,                          # style
    ...
    my $reg_ok = $RegisterClassW->Call($packed);
    die "RegisterClassW failed: $^E" unless $reg_ok;
...
sub guiPrompt {
    ...
    my $class_name_w = to_wide_bytes("#32770");
    my $title_w = to_wide_bytes($title);
    register_wndclass($hInstance, $class_name_w);
